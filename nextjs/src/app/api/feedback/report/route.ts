...
const Schema = z.object({
  message: z.string().trim().min(3, 'Tell us a sentence or two of what happened.').max(4000),
...
export async function POST(req: NextRequest) {
  ...
  const { message, path, digest, context } = parsed.data
  ...
  try {
    ...
    const parts = {
      eyebrow: 'User report',
      heading: `Error feedback — ${reporterLabel}`,
      bodyHtml: `
    ...
    await sesClient.send(new SendEmailCommand({
      ...
      Content: {
        Simple: {
          Subject: { Data: `[Feedback] ${path}`, Charset: 'UTF-8' },
          Body: {
