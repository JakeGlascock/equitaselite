...
// Pitch-deck links with a paired preview token. Each deck mint also
// auto-mints a preview token that walks the recipient through the
// chosen demo profile; the deck's __PREVIEW_URL__ placeholder is
// substituted with /preview/<paired-token> at render time. Revoking
// the deck cascades to revoking the paired preview.
export default function DeckTokensPanel({ demoProfiles }: { demoProfiles: DemoProfile[] }) {
  ...
  async function mint(e: React.FormEvent) {
    ...
    try {
      ...
      const url = `${window.location.origin}/deck/${data.token}`
  ...
  async function revoke(token: string) {
    if (!confirm('Revoke this deck link? Anyone holding it will be cut off immediately.')) return
    try {
      const res = await fetch('/api/admin/deck-tokens', {
        method:  'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-6">
      <form onSubmit={mint} className="space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label className="block">
            <span className="text-[10px] font-data uppercase tracking-widest text-ee-muted">Paired preview views as</span>
            <select
              value={profileId}
      ...
      {justMinted && (
        <div className="glass-panel border-ee-gold/40 p-4 space-y-2">
          <p className="text-xs text-ee-gold font-data uppercase tracking-widest">Link ready</p>
      ...
      <div>
        <p className="text-[10px] font-data uppercase tracking-widest text-ee-muted mb-2">
          Existing deck links {loading && '· loading…'}
        </p>
        {tokens.length === 0 ? (
          <p className="text-xs text-ee-muted">No deck links yet.</p>
        ) : (
